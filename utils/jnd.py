...
def get_jnd_map(dir_path):
    ...
    for img_name in os.listdir(dir_path):
        ...
        _, _, c = img.shape
        jnd_map = np.zeros_like(img)
        for i in range(c):
            img_gray = img[:, :, i]
            # luminance adaptation
            jnd_LA = func_bg_lum_jnd(img_gray)  # Equ. 13

            # luminance contrast masking
            L_c = func_luminance_contrast(img_gray)  # Equ. 7
            alpha = 0.115 * 16
            beta = 26
            jnd_LC = (alpha * np.power(L_c, 2.4)) / (np.power(L_c, 2) + beta ** 2)  # Equ. 11

            # content complexity
            P_c = func_ori_cmlx_compute(img_gray)  # Equ. 6
            a1 = 0.3
            a2 = 2.7
            a3 = 1
            ...
            # edge protection
            edge_protect = func_edge_protect(img_gray)
            ...
            # JND map
            jnd_map_temp = jnd_LA + jnd_VM - 0.3 * np.where(jnd_LA < jnd_VM, jnd_LA, jnd_VM)  # Equ. 14
            jnd_map_temp = to_uint8(jnd_map_temp)
            # plt.imshow(jnd_map_temp,'gray')
            # plt.show()
            jnd_map[:, :, i] = jnd_map_temp
        jnd_map = np.uint8(jnd_map)

        # dilate jnd maps
        jnd_map = cv2.cvtColor(jnd_map, cv2.COLOR_RGB2BGR)
        kernel = np.ones((5, 5), np.uint8)
        dilated_jnd = cv2.dilate(jnd_map, kernel, iterations = 3)
        jnd_map = cv2.cvtColor(dilated_jnd, cv2.COLOR_BGR2RGB)
        jnd_map = np.where(jnd_map > 0.25 * 255 , 255, 0)
        # jnd_map = np.where(jnd_map > 0.25 * 255 , jnd_map, 0)
        # jnd_map = np.where(jnd_map > 0.75 * 255 , 255, jnd_map)
        all_jnd_maps.append(jnd_map)
    all_jnd_maps = np.array(all_jnd_maps, dtype=np.uint8)
    # all_jnd_maps = sigmoid(all_jnd_maps)
    return all_jnd_maps
...
#     jnd_map = get_jnd_map("../save_figs/noise_0423/noise_05/0012_01_ori.png")
#     plt.imshow(jnd_map)
#     plt.show()
