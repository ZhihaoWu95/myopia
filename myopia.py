...
from utils.jnd import get_jnd_map
import cv2


logger = get_logger(__name__)

jpeg_transforms = transforms.Compose(
            [
...
def load_jnd_maps(data_dir, size=512, center_crop=True) -> torch.Tensor:
    print("Creating jnd maps...")
    image_transforms = transforms.Compose(
        [
            transforms.Resize(size, interpolation=transforms.InterpolationMode.BILINEAR),
            transforms.ToTensor(),
    ...
    print("jnd maps creating from ", data_dir)
    jnd_maps = get_jnd_map(data_dir)
    images = [image_transforms(Image.fromarray(jnd_maps[i])) for i in range(len(jnd_maps))]
    images = torch.stack(images)
    print("jnd maps created with shape: ", images.shape)
    return images
...
def pgd_attack(
    ...
    for step in range(num_steps):
        perturbed_images_last = perturbed_images.clone()
        
        if args.eot is True:
            if np.random.random() < 0.3:
                # blur
                k = k_list[np.random.randint(0, 4)]
                perturbed_images.data = transforms.functional.gaussian_blur(perturbed_images.data, (k, k), (1, 1))
            if np.random.random() < 0.3:    
                # jpeg
                q = q_list[np.random.randint(0, 4)]
                for im in range(perturbed_images.shape[0]):
                    img_data = copy.deepcopy(perturbed_images.data[im])
                    img_data = (img_data * 127.5 + 128).clamp(0, 255).to(torch.uint8).permute(1, 2, 0).cpu().numpy()
                    img_data = cv2.cvtColor(img_data, cv2.COLOR_RGB2BGR)
                    img_encode = cv2.imencode('.jpg', img_data, [int(cv2.IMWRITE_JPEG_QUALITY), q])[1]
                    img_decode = cv2.imdecode(img_encode, cv2.IMREAD_COLOR)
                    img_decode = cv2.cvtColor(img_decode, cv2.COLOR_BGR2RGB)
                    img_encode_tensor = jpeg_transforms(Image.fromarray(img_decode.astype(np.uint8)))
                    perturbed_images.data[im] = img_encode_tensor.data
        ...
        noise = torch.randn_like(latents)
        bsz = latents.shape[0]
        # Sample a random timestep from guassian distribution
        timesteps = torch.abs(torch.normal(0, 0.25, size=(bsz,), device=latents.device))
        timesteps = torch.clamp(timesteps, 0, 0.999) * noise_scheduler.config.num_train_timesteps
        ...
        adv_images = perturbed_images_last - alpha * perturbed_images.grad.sign()

        # use jnd method
        eta = adv_images - original_images
        eta = gamma * jnd_maps * eta
        eta = torch.clamp(eta, min=-eps, max=+eps)
        perturbed_images = torch.clamp(original_images + eta, min=-1, max=+1).detach_()

        if (step + 1) % 10 == 0:
            print(f"PGD loss - step {step}, loss: {loss.detach().item()}, pred loss: {loss.detach().item()}")
...
def main(args):
    ...
    for img_pixel, img_name in zip(noised_imgs, img_names):
        save_path = os.path.join(save_folder, f"{i+1}_{img_name}")
        Image.fromarray(
